...
// filter Finals
const finals2014 =
    fifaData.filter(item => item.Year === 2014 && item.Stage === "Final")
 ...
 * callback function `getFinals`, and returns an array called `years` containing
 ...
 * of each `finals` game. Return the name of all winning countries in an array
 * called `winners` */
...
function getWinners(data, getFinals) {
  ...
  return getFinals(data).map(match => match["Home Team Goals"] >
...
/* Task 5: Implement a higher-order function called `getWinnersByYear` that
...
function getWinnersByYear(data, getYears, getWinners) {
  ...
  return winners.map((item, index) =>
                         `In ${years[index]}, ${item} won the world cup!`);
};

console.log("Task 6:", getWinnersByYear(fifaData, getYears, getWinners));

/* Task 6: Create a function called `getCountryWins` that takes the parameters
...
//         if (match["Home Team Goals"] > match["Away Team Goals"]) {
//             return { initials: match["Home Team Initials"], name: match["Home
//             Team Name"] }

//         } else {
//             return { initials: match["Away Team Initials"], name: match["Away
//             Team Name"] }
//         }
//     })

//     const winName = winnerInitials.filter(name => name.initials ===
//     teamInitials)

//     return `${winName[0] ? winName[0].name : teamInitials} has
//     ${winName.length} World Cup win${winName.length === 1 ? '.' : 's.'}`
// };
 ...
 * `data` and returns the the average number of home team goals and away team
